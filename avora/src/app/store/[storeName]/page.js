...
import { getStoreByName, getProductsByStoreName } from '../../../lib/firestore'
...
const HARDCODED_STORE_INFO = {
  ...
  'SmartMinds': { emoji: '🎓', category: 'Tutoring', location: 'Comes to you', rating: 4.8, reviews: 93, description: 'Expert tutoring in Maths, Physics, Chemistry and more. Flexible hours for busy students.', shield: 90, hours: '12pm - 6pm daily' },
...
export default function StorePage({ params }) {
  ...
  // State for data that comes from localStorage
  const [storeProducts, setStoreProducts] = useState([])
  const [info, setInfo] = useState(null)
  const [loaded, setLoaded] = useState(false)

  useEffect(() => {
    const hardcodedInfo = HARDCODED_STORE_INFO[storeName]
    const hardcodedProducts = HARDCODED_PRODUCTS.filter(p => p.store === storeName)

    if (hardcodedInfo) {
      setInfo(hardcodedInfo)
      setStoreProducts(hardcodedProducts)
      setLoaded(true)
      return
    }
      ...
      try {
        ...
        if (store) {
          setInfo({
            ...
            location: store.location || 'Campus',
            rating: store.rating || 5.0,
            reviews: store.reviews || 0,
            description: store.description || 'Welcome to this store on Avora.',
            shield: store.shield || 70,
            hours: store.hours || 'Contact seller',
            whatsapp: store.whatsapp || '',
            ownerName: store.ownerName || '',
            isLocalStore: true,
            storeId: store.id,
          })
          const prods = await getProductsByStoreName(storeName)
  ...
  if (!loaded) {
    return (
      <main style={{ padding: '5rem 2rem', textAlign: 'center' }}>
        <p style={{ fontSize: '0.8rem', color: '#6a6a6a' }}>Loading store...</p>
      </main>
    )
  }
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#fafafa' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '2rem', flexWrap: 'wrap' }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexWrap: 'wrap', marginBottom: '0.4rem' }}>
                <h1 style={{ fontFamily: 'Georgia, serif', fontSize: 'clamp(1.5rem, 4vw, 2.2rem)', fontWeight: '400', color: '#fafafa' }}>
                  {storeName}
                </h1>
                <span style={{
              ...
              {info.ownerName && (
                <p style={{ fontSize: '0.6rem', color: 'rgba(255,255,255,0.4)', marginBottom: '0.5rem' }}>
                  👤 {info.ownerName}
        ...
        {activeTab === 'listings' && (
          storeProducts.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '4rem 2rem', border: '0.5px solid rgba(0,0,0,0.09)' }}>
              <div style={{ fontSize: '3rem', marginBottom: '1rem' }}>📭</div>
              <h2 style={{ fontFamily: 'Georgia, serif', fontSize: '1.3rem', fontWeight: '400', marginBottom: '0.5rem' }}>No listings yet</h2>
              <p style={{ fontSize: '0.72rem', color: '#6a6a6a' }}>This store hasn't added any products yet.</p>
            </div>
          ) : (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '1.5rem', overflow: 'visible' }}>
              {storeProducts.map(product => (
                <div
                  ...
                  <div style={{ padding: '1rem' }}>
                    <p style={{ fontSize: '0.55rem', color: '#C9A84C', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '0.25rem' }}>
                      {product.category}
                    </p>
                    <h3 style={{ fontSize: '0.85rem', fontWeight: '600', color: '#0a0a0a', marginBottom: '0.5rem', lineHeight: 1.4 }}>
                      {product.name}
                    </h3>
                    <p style={{ fontFamily: 'Georgia, serif', fontSize: '1rem', color: '#0a0a0a', marginBottom: '1rem' }}>
                      {product.type === 'service' ? 'From ' : ''}₦{product.price.toLocaleString()}
                    </p>
                    <div style={{ display: 'flex', gap: '0.5rem' }}>
                      <button
                        ...
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="1.5">
                          <path d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                        </svg>
                        Message
                      </button>
        ...
        {activeTab === 'about' && (
          <div style={{ maxWidth: '600px' }}>
            ...
            {[
              ...
              ...(info.whatsapp ? [{ label: 'WhatsApp', value: info.whatsapp }] : []),
              ...(info.ownerName ? [{ label: 'Owner', value: info.ownerName }] : []),
            ].map(row => (
              <div key={row.label} style={{ display: 'flex', justifyContent: 'space-between', padding: '0.85rem 0', borderBottom: '0.5px solid rgba(0,0,0,0.06)' }}>
      ...
      {/* REVIEWS MODAL */}
      {reviewsOpen && (
        <div onClick={() => setReviewsOpen(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 500, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
          <div onClick={e => e.stopPropagation()} style={{ background: '#fff', maxWidth: '500px', width: '100%', padding: '2rem', maxHeight: '80vh', overflowY: 'auto' }}>
            ...
            <div style={{ background: '#f5f3ee', padding: '1.5rem', textAlign: 'center', marginBottom: '1.5rem' }}>
              <div style={{ fontFamily: 'Georgia, serif', fontSize: '3rem', color: '#0a0a0a' }}>{info.rating}</div>
              <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '0.4rem' }}><StarRating rating={info.rating} /></div>
            ...
            {info.reviews > 0 ? [
              ...
              <div key={i} style={{ padding: '1rem 0', borderBottom: '0.5px solid rgba(0,0,0,0.06)' }}>
                ...
                <p style={{ fontSize: '0.72rem', color: '#0a0a0a', lineHeight: 1.6, marginTop: '0.25rem' }}>{review.comment}</p>
              </div>
            )) : (
              <p style={{ fontSize: '0.72rem', color: '#6a6a6a', textAlign: 'center', padding: '1rem 0' }}>
            ...
            <div style={{ marginTop: '1.5rem' }}>
              ...
              <button
                onClick={() => requireAuth(() => { showToast('✅ Review submitted!'); setReviewsOpen(false) })}
                style={{
